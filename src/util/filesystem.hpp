...
namespace Filesystem {
  ...
  std::string save_image_as_png(const std::string filename, const float* pixels, const size_t w, const size_t h, const float downsample_factor = 1.0f, const TextureFormat fmt = TextureFormat::RGB32F) {
    assert(downsample_factor >= 1.0f && "Downsample factor must be larger or equal to 1.0");
    assert(false && "Unimplemented PNG support");
  }

  std::string save_image_as_ppm(const std::string filename, const float* pixels, const size_t w, const size_t h, const float downsample_factor = 1.0f, const TextureFormat fmt = TextureFormat::RGB32F);

  std::string save_image_as(const std::string filename, const ImageFormat img_fmt, const float* pixels, const size_t w, const size_t h, const float downsample_factor = 1.0f, const TextureFormat texture_fmt = TextureFormat::RGB32F);

  /// Saves the text passed in a file at filepath
  void save_text_in_file(const std::string& filepath, const std::string& txt) {
    std::ofstream file;
