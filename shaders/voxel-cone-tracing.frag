...
void main() {
  ...
  const vec3 specular = mix(F0, base_color, metallic);
  const vec3 diffuse  = base_color * (vec3(1.0) - F0) * (1.0 - metallic);

  // Diffuse cones
  color += trace_cone(origin, normal, roughness_aperature * roughness);
  #define NUM_DIFFUSE_CONES 0
  ...
  color += specular * trace_cone(origin, reflection, metallic_aperature).rgb; 
